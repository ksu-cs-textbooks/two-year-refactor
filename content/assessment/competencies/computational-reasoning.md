...
| B8 | CS-208 | Performance analysis on a real deployed bottleneck; OS/concurrency model analysis |
...
| Adaptation | Performance investigation (CS-208) — analyze real system behavior where empirical results diverge from theory |
...
| B7 | CS-206/207 | Choosing the right abstraction for the shape of the problem (graph vs. relational vs. document) |
| B8 | CS-208 | Actor model and containers as deployment abstractions; API versioning as compatibility abstraction |
...
| B6 | CS-203 | Testing as formal verification; property-based testing; test suite as formal specification |
| B7 | CS-206 | Algorithm correctness arguments; constraint/logic model (Prolog-style) |
| B8 | CS-208 | Formal + empirical reconciliation — construct the argument for why Big-O prediction diverges from observed behavior |
...
| Adaptation | Formal + empirical reconciliation (CS-208) — apply formal reasoning where the textbook model breaks down; construct the argument for why it breaks down |
...
| B7 | CS-206 | Dijkstra vs. A\*, BFS vs. DFS, Kruskal vs. Prim — multiple algorithms for the same problem with different applicability conditions |
| B7 | CS-206/207 | Design Review — defend algorithm and data model choices to peers and faculty |
| B8 | CS-208 | Production bottleneck — evaluate algorithm choices where formal analysis and empirical behavior diverge; concurrency model selection |
