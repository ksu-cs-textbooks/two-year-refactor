...
| B8 | CS-208 | Primary home — full memory hierarchy (cache, locality, virtual memory, thrashing); OS internals (processes vs. threads, preemptive vs. cooperative scheduling, kernel vs. user space); concurrency model behavior; performance analysis on a real deployed bottleneck |
...
| Adaptation | Production system analysis (CS-208) — explain a live performance bottleneck using monitoring data; reconcile theoretical and empirical behavior |
...
| B6 | CS-203 | Test runner configuration — setting up automated test environments, not just writing tests |
| B6 | CS-205 | Collaborative toolchain — shared Git workflow configuration, branch protection, code review tooling |
| B8 | CS-208 | Primary home — containers as environment abstraction; dependency management; production configuration; monitoring and logging tooling; hardening before live |
...
| Adaptation | Container-based deployment (CS-208) — configure a production environment with real security, dependency, and operational constraints |
...
| B6 | CS-203 | Test suite as pre-deployment validation gate |
| B6 | CS-205 | Git PR workflow as release mechanism — merge to main as a deployment trigger; semantic versioning as release communication |
| B8 | CS-208 | Primary home — containerized deployment; monitoring and logging; operational runbooks; harden-before-live; API versioning as backward-compatibility contract; fault-tolerance (actor model, circuit breakers); performance analysis under real load |
| B8 | CS-209 | System Integration Project — deploy for real user acceptance testing; blameless postmortem as operational retrospective |
