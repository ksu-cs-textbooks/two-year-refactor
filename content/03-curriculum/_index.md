...
| B2 | CS-104 Data Transformation & Manipulation; CS-105 Code Reading & Repair; CS-106 Web Foundations & Data-Driven Rendering | — |
| B3 | CS-107 Object-Oriented Programming I; CS-108 Object-Oriented Programming II; CS-109 Abstract Data Types | MATH-102 Discrete Math: Counting Finite Configurations |
| B4 | CS-110 Trees, Hashing & Hierarchies; CS-111 Systems & APIs; CS-112 Algorithmic Complexity | — |
| B5 | CS-201 SQL Fundamentals; CS-202 Database Design | MATH-103 Discrete Math: Recursive and Modular Computation; STAT-101 Descriptive Statistics & Data Summarization |
| B6 | CS-203 Software Testing & Validation; CS-204 Security Fundamentals; CS-205 Collaborative Development | — |
...
| B8 | CS-208 Deployment & Operations; CS-209 Accessible & Human-Centered Design; CS-210 Data Analysis & Responsible AI | — |

**Totals:** 22 CS credits + 5 external (co-designed) credits = **27 credits currently allocated.** The original target was 26 (22 CS + 4 discrete math). The count now exceeds it because STAT-101 has been added and further additions are agreed-in-principle (the rest of the statistics sequence, a linear-algebra course, and a 2-credit OSI networking unit). **The credit budget is deliberately left soft** pending the cross-block optimization pass — see open questions.

## The fourteen threads

Competencies belong to the program, not to individual courses; courses are contexts in which program-level competencies are demonstrated. Fourteen named threads run through the blocks, in three kinds that mature in three different ways:

### Spirals (9) — depth increases through the topic’s own escalation
1. **Data Structures & Representation** — primitive → sequence → hierarchy → persistent → relational → graph → document. The cleanest spiral.
2. **Program Correctness / Code Comprehension** — make it work → understand others’ → organize → verify → defend.
3. **APIs & Networked Systems** — consume → build → integrate → operate.
4. **HCI / Accessible & Human-Centered Design** — carries two embedded throughlines (accessibility + data visualization); checklist compliance → requirements-driven design.
5. **Algorithmic Thinking & Complexity** — theory → graph optimization → real bottleneck.
6. **Git & Version Control** — read history → commit/revert → solo branching → collaborative.
7. **Computational Models & Error Handling** *(flagship)* — one model → many; “how does this model handle failure” as the connective idea; ends comparing fault-tolerance philosophies.
8. **Testing & Formal Verification** — informal reasoning → regression → testing AND verification, dual-tracked.
9. **Boundaries & Contracts** *(meta-thread)* — a promise at a boundary; unifies APIs, schemas, trust boundaries, versioning, and verification. The strongest expression of the systems-thinking ethos.
10. **Sociotechnical Structure (Conway’s Law)** — systems mirror the orgs that build them; observe → live → reflect. *(Counted among the spirals; professional/human family.)*

### Lenses (3) — a standing question/practice applied wherever relevant; scope grows with capability
- **Security, Privacy & Ethical Reasoning** — asked wherever content raises it; formalized once (B6, the likely ABET ethics anchor).
- **Documentation Practice** — register changes with audience (self → reviewer → teammate → operator); the program’s first example competency.
...
*(The taxonomy lists ten spirals including Sociotechnical; “nine technical spirals + one human-structure spiral” is the precise framing. Either way: 10 spiral-type + 3 lenses + 1 bounded = 14 threads.)*
...
1. **Credit budget.** Allocation now stands at 27 credits vs. an original 26 target, and agreed-in-principle additions (rest of the stats sequence, linear algebra, OSI networking) will push it higher. A cross-block optimization pass is needed to reconcile — grow the total, displace content, or move items outside the required core.
2. **Thread legibility.** Fourteen threads is a lot for a faculty member teaching one 1-credit course to hold. The threads interconnect (the convergence points above are evidence they’re integrated, not fragmented), but the program needs a deliberate answer to “how does a course owner see which threads run through their course, and what each expects, without a decoder ring?” The block files are a first attempt; a per-course thread-tag scheme may be needed.
...
4. **Statistics sequence.** STAT-101 is placed (B5). STAT-102–104 (probability, distributions/sampling, regression) are unplaced. The AI degree also needs statistical inference, linear algebra, probability, data visualization, and optimization — not all of which live in the stats courses.
